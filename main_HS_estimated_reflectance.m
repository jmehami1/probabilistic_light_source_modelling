...
if strcmp(testDataDir, 'lamb_carcass')
    skipSteps = true;
end
...
if usingTestData
    ...
    lightTrigFilePath = fullfile(sourDir, 'pt_light.yaml');
    if ~exist(lightTrigFilePath, 'file')
        error('light source YAML trigulation file does not exist');
    end
    
    gpLightMappingFile = fullfile(sourDir, 'gp_lightsrc_optm.mat');
    if ~exist(gpLightMappingFile, 'file')
        error('GP lighting model has not been built for test_data or missing MAT file');
    end
...
ligSrcYaml = yaml.ReadYaml(lightTrigFilePath);
...
if ~skipSteps
    ...
    % load MAT file containing surface normal image and binary pixel mask
    load(fullfile(sourDir, 'object_shape_info.mat'));
    ...
    for imgLoop = 1:numImages
        %check if any valid pixel exist for this line
        curValidPixelMask = hsHypCubeRayIntersecMask(:,imgLoop);
        
        if ~any(curValidPixelMask)   
            continue;
        end
        
        numPts = sum(curValidPixelMask);
        numPixPerLine(imgLoop) = numPts;
        vPixs = find(curValidPixelMask);
        
        %Spectral Radiance for each pixel 
        curNormRadiLine = RadiImgCell{imgLoop}';
        curNormRadiLine = curNormRadiLine(curValidPixelMask, :);
        
        surfNormLine = squeeze(hsHypCubeSurNormMat(curValidPixelMask, imgLoop, :));

        pntFLine = squeeze(hsHypCubeXYZMat(curValidPixelMask, imgLoop, :));
        
        pixRadianceData(numData + 1:numData + numPts, :) = [
            vPixs, pntFLine, surfNormLine, curNormRadiLine
            ];
        
        numData = numData + numPts;
...
imwrite(uint8(((surfImg+1)./2).*255), 'normal_map.png');
...
imwrite(uint8(((ligImg+1)./2).*255), 'light_source_direction_map.png');
...
subplot(1,3,3)
imshow(lDotnImg);
imwrite(lDotnImg, 'l_dot_n_map.png');
...
binMaskG = bwmorph(binMaskG, 'clean', Inf);
figure;imagesc(binMaskG);

binMaskG = bwareaopen(binMaskG, 50);
finalReflectanceMaskFile = fullfile(sourDir, 'reflectanceMask.mat');
save(finalReflectanceMaskFile, 'binMaskG');
...
%     radiantIntenHyperCubeMasked = bsxfun(@times, radiantIntenHyperCube, binMaskG);
    ...
    hyperCube = S;
        ...
        imwrite(mat2gray(k), fullfile(saveDir, [method, '_k.png']));
    imwrite(mat2gray(g), fullfile(saveDir, [method, '_g.png']));
    
    
    for i = 1:3
        falseChanGray = double(hyperCube(:,:,channels(i)))./uint16Max;
        falseChan = mat2gray(falseChanGray);
        falseImg(:,:,i) = falseChan;
    end
    
    % falseImg = double(falseImg)./uint16Max;
    
    imshow(falseImg)
    
    imwrite(falseImg, fullfile(saveDir, [method, '_Refl.png']));
...
if runKrebsShape
    ...
    [S, k, g] = ReflectanceEstimationKrebs(normRadianceHyperCube,radiantIntenEstimatedBand, binMaskG, lDotnImg, 3, 0.01);
